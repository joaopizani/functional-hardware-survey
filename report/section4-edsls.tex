...
\label{sec:edsls}
    ...
    \label{subsec:lava}
        ...
        \label{subsubsec:lava-circuits}
            ...
            adders. To get used to the wat in which circuits are described in Lava, let us first
            ...
            Besides modelling the three circuits in Lava, we also simulated them. The definition of
    ...
    instance Eq (Signal a) where
        Signal (Symbol r1) == Signal (Symbol r2) = r1 == r2
            ...
            in GHC 7.6 and being improved for GHC 7.8) would make modelling in Lava \emph{safer} and
            more comfortable.
    ...
    \label{subsec:forsyde}
        ...
        hardware modelling\cite{forsyde1999}. The ForSyDe approach per se has several significant
        ...
        \begin{description}
            ...
            \item[Process constructor] Every ciruit in ForSyDe (even simplest combinational ones) is
        ...
        \label{subsubsec:forsyde-mocs}
            ...
            designs nowadays having a global clock. Also, all of our studied circuits were modelled
            ...
            On section \ref{subsec:forsyde-synchprocs} We take a closer look at synchronous process
            constructors, as well as the mechanism by which ForSyDe translates (small fragments of)
            Haskell source code into the ``building blocks'' of synchronous systems in VHDL.

            On section \ref{subsec:forsyde-circuits} we proceed to expose the circuits we modeled in
            ForSyDe, using the models to make a comparative analysis of ForSyDe with the other
            EDSLs.
    ...
    counter = sourceSY "counterProc" incrFunc 0
        ...
        \label{subsubsec:forsyde-circuits}
            ...
            modelling the 3 circuits used as case-studies. ForSyDe has a peculiar ``dual'' nature,
            ...
            Because of this dual nature of ForSyDe, when modelling the case-study circuits we
            ...
            \begin{description}
                \item[High-level] A model that uses Haskell constructs inside the process functions
                    ...
                    syntax tree of process functions for synthetization, and therefore \textbf{can
            ...
            using 16-to-1 and 4-to-1. We only show the first two degress of the hierarchy in listing
            ...
            When finished modelling a circuit in ForSyDe, we ``wrap it up'' in a \emph{system
            ...
            can be seen in listing \ref{lst:cpu-parts}. The code for the ForSyDe version of the
            ...
            version available on Hackage\cite{forsyde-3.1.1-hackage} dates from 2010, and on the
    ...
    Inductive vec A : nat -> Type :=
        ...
        \label{subsubsec:coquet-modelling}
            ...
            exposed further ahead. Now let's take a look at a half-adder described in Coquet:
    ...
    Class Fin A := {
        ...
        \label{subsubsec:coquet-semantics}
            ...
            and it describes solely which are the parts that the circuit is make of and how they are
    ...
    Semantics y middles outs
            ...
            correctness is making sure that the circuit we are modelling is \emph{equivalent} (in a
        ...
        \label{subsubsec:coquet-improv}
            The approach to circuit modelling is \emph{very} generic, and leaves room for extension
            in several aspects, even though the library is already provided with some reasnoable
            deafults.

            For example, the definition of the \emph{meaning relation} for circuits (by induction on
            circuit structure) is parameterized by the \emph{type \texttt{T} of was is carried in
                the wires}. Even though the library already provides examples for booleans and
            streams of booleans, it could also be interesting to add cases for dealing with some
            three-valued logics, or a case for IEEE1164's std\_logic type, used often in VHDL.

            Another interesting point is that Coquet defines a \texttt{stream} type family, and then
            proceeds to define several interesting function over \texttt{stream}, as well as an
