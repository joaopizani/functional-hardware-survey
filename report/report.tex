...
\begin{document}
    ...
    Qed.
                ...
                \texttt{NOT} circuit is a serial composition of a \texttt{Fork2} circuit (which
                simply splits the input into two identical copies) and a \texttt{NOR} circuit, which
                is the underlying fundamental gate in this case.
                ...
                and tags, are exposed in the next sections. Now let's take a look at a half-adder
                described in Coquet:
                \begin{verbatim}
    Program Definition HADD a b s c : ℂ ([:a] + [:b]) ([:s] + [:c]) :=
        Fork2 ([:a] + [:b]) |> (XOR a b s & AND a b c).

    Instance HADD_Implement {a b s c} :
        Implement (HADD a b s c) _ _
        (fun (x : bool * bool) => match x with
                                  | (a, b) => (xorb a b, andb a b)
                                  end.
        ).
        Proof.
